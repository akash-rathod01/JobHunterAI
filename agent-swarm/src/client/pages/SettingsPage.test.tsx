...
describe("SettingsPage", () => {
  ...
  it("allows saving on RxResume availability warnings and keeps the inline warning visible", async () => {
    ...
    vi.mocked(api.validateRxresume).mockResolvedValue({
      valid: false,
      message:
        "J-JobHunterAI could not verify Reactive Resume because the instance is unavailable right now.",
      status: 0,
    });
    ...
    expect(toast.info).toHaveBeenCalledWith(
      "Settings saved, but J-JobHunterAI could not verify Reactive Resume because the instance is unavailable.",
    );
  });
