...
export interface JobHunterAICrawlProgressPayload {
  phase: CrawlPhase;
  currentUrl?: string;
...
interface JobHunterAICrawlProgressState {
  listPagesProcessed: number;
  listPagesTotal?: number;
  jobCardsFound: number;
  jobPagesEnqueued: number;
...
const state: JobHunterAICrawlProgressState = {
...
function emit(): void {
  ...
  const payload: JobHunterAICrawlProgressPayload = {
    phase: state.phase,
...
export function initJobHunterAIProgress(listPagesTotal: number): void {
